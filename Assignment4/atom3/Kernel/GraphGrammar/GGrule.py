...
class GGrule (ASGNode, ATOM3Type):
   def __init__(self, executionOrder = None):
      ...
      self.NAC = None                                   # negative application condition
   ...
   def addNewConnections (self, idGraph, node, collection, newNodes):
     """ 
     Add connections appearing in RHS (node) but not in LHS (node._matched)
     Returns a list of newly added connections (Subclasses of ASGNode)
     """
     newConnectionList = []
       
     # Incomming connections
     if( collection == "in"):
       # Go through all incomming connections of node: el1 = connection
       for el1 in node.in_connections_:     
         # Now search 'el1' in node._matched...
         # If it does not appear in LHS, add connection in host graph...
         if( not self.getMatched(idGraph, el1) 
           in self.getMatched(idGraph, node).in_connections_):
                  
           # See if the target node is new...
           if self.getMatched(idGraph, el1) in newNodes:
             # We have to find out wich element if 
             #  node._matched[0]._matched we have to look at
             m = self.getMatched(idGraph, self.getMatched(idGraph, node))
             m.in_connections_.append(self.getMatched(idGraph, el1))
             link = self.getMatched(idGraph, el1)
                        
           # Not new, so we have moved a connection, but do it only on this
           # side, as the iteration to change connections is on all nodes!
           else: 
             m = self.getMatched(idGraph, self.getMatched(idGraph, node))
             m.in_connections_.append(
               self.getMatched(idGraph, self.getMatched(idGraph, el1)))
             link = self.getMatched(idGraph, self.getMatched(idGraph, el1))
           newConnectionList.append(link)
     ...
     # Outgoing connections     
       ...
       for el1 in node.out_connections_:     
         # now search 'el1' in node._matched...
         if( not self.getMatched(idGraph, el1) 
           in self.getMatched(idGraph, node).out_connections_):
                   
           # Does not appear in LHS!, add connection in host graph...
           # See if the target node is new...
           if self.getMatched(idGraph, el1) in newNodes:
             m =  self.getMatched(idGraph, self.getMatched(idGraph, node))
             m.out_connections_.append(self.getMatched(idGraph, el1))
             link = self.getMatched(idGraph, el1)
           ...
           # side, as the iteration to change connections is on all nodes!
             ...
             m = self.getMatched(idGraph, self.getMatched(idGraph, node))
             m.out_connections_.append(self.getMatched(idGraph, 
               self.getMatched(idGraph, el1)))
             link = self.getMatched(idGraph, self.getMatched(idGraph, el1))
           newConnectionList.append(link)
   ...
   def replaceSides (self, parent, homGraphTuple, realGraph, moveEntities, graphics):
      ...
      for node in homGraph:                             			# for each node in graph
         ...
         if rhsNode:                                    			# node exists...
            ...
            # see HierarchicalASGNode.py for more info
            if(node.isHierarchicalNode()):
              node._removeNodeFromHierarchyTopLayer()
      ...
      for nt in self.RHS.listNodes.keys():              	# for all kind of nodes...
         for node in self.RHS.listNodes[nt]:            	# for all nodes of type <nt>
            if self.getMatched(idHomGraph,node) in newNodes:    # it is a new node, just copy mapped connections from RHS
               ...
               # ...........................................................................
               newSemanticConnections += \
                 self.addNewConnections (idHomGraph, node, "in", newNodes)
               # ...........................................................................
               # b) in out_connections_
               # ...........................................................................
               newSemanticConnections += \
                 self.addNewConnections (idHomGraph, node, "out", newNodes)
               # ...........................................................................               
...
##      print 'applied AO'
  ...
  #    print 'node', node.__class__.__name__
    ...
    # Only one reference position!
    else:
      from random import randint
      xDir = [1, -1][randint(0,1)]
      yDir = [1, -1][randint(0,1)]
      xMag = randint(40,140)
      yMag = randint(40,140)
      return (xCoords[0] + xDir * xMag, yCoords[0] + yDir * yMag)
