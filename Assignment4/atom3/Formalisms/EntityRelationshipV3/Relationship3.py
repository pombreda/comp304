"""
__Relationship3.py_____________________________________________________
...
class Relationship3(ASGNode, ATOM3Type):

   def __init__(self, parent = None):
      ASGNode.__init__(self)
      ATOM3Type.__init__(self)
      self.graphClass_ = graph_Relationship3
      self.isGraphObjectVisual = True
      self.parent = parent
      self.name=ATOM3String('Relationship_')
   ...
   def clone(self):
      cloneObject = Relationship3( self.parent )
   ...
   def postCondition (self, actionID, * params):
      if actionID == self.CONNECT:
         res = self.relationships2classes(params)
         if res: return res
   ...
   def relationships2classes(self, params):
