...
#   Source: D:/EclipseWorkspace/atom3/Formalisms/FSB_Buttons/FSB_Code\ButtonBehaviour_MDL.des
#   Date:   May 29, 2006
#   Time:   12:52:43
...
class ButtonBehaviour_MDL(StateMachine):
  ...
  StateNames=["Activate Button*",
              "Default",
              "Idle" 
             ]
  ParentTable=[-1,  # Activate Button* -- parent (None)
  ...
  LeafStateTable=["Activate Button*",
                  "Default",
                  "Idle" 
                 ]
  ...
  Hierarchy=[[0, 0, 0],  # substates of state Activate Button*
  ...
  def handleEvent(self, se, params=[], cond=None, scheduler=None):
    ...
    if e==0: # event "(create)*"
	  ...
	  self.clearEnteredStates()
          ...
          self.applyMask(ButtonBehaviour_MDL.OrthogonalTable[1], table)
          handled=1
	  ...
	  self.clearEnteredStates()
          ...
          self.runEnterActionsForStates(self.StatesEntered, 1)
          self.applyMask(ButtonBehaviour_MDL.OrthogonalTable[2], table)
          handled=1
	  ...
	  self.clearEnteredStates()
          ...
          self.runEnterActionsForStates(self.StatesEntered, 1)
          self.applyMask(ButtonBehaviour_MDL.OrthogonalTable[0], table)
          handled=1
    if cond and self.Parent==None:  # Top-level model
      cond.release()
    if not handled and e>=0 and (scheduler==self or scheduler==None) and ButtonBehaviour_MDL.RescheduleTable[e]>=0:
      self.addSchedule(ButtonBehaviour_MDL.RescheduleTable[e], ButtonBehaviour_MDL.IntervalTable[e], ButtonBehaviour_MDL.EventNames[e], scheduler)
    return handled
      ...
      if dest==0:
        if history_type!=2 or self.check_history(-1):
          if history_type!=2 or self.check_history(0):
            ...
            self.addInState(0)  # move into leaf state "Activate Button*"
      elif dest==1:
        if history_type!=2 or self.check_history(-1):
          if history_type!=2 or self.check_history(1):
            self.recordEnteredState(1)
    ...
    if isinstance(state, int):
      ...
      for i in range(ButtonBehaviour_MDL.StateNum):
        if ButtonBehaviour_MDL.LeafStateTable[i]==None:
          continue
        if state==ButtonBehaviour_MDL.LeafStateTable[i] and self.Submodels[i]==None:
          return 1
        elif startsWith(state, ButtonBehaviour_MDL.LeafStateTable[i]+".") and self.Submodels[i]!=None:
          SubmodelState=state[ButtonBehaviour_MDL.LeafStateTable[i].length()+1:]
...
if __name__=="__main__":
  ButtonBehaviour_MDL.main(sys.argv)