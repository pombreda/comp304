"""
__ButtonBehaviour_MDL.py_____________________________________________________

Automatically generated AToM3 Model File (Do not modify directly)
Author: Denis Dube
Modified: Mon May 29 12:52:39 2006
...
def ButtonBehaviour_MDL(self, rootNode, DChartsRootNode=None, AnnotationRootNode=None):
    ...
    # name
    self.obj39.name.setValue('Activate Button*')
    ...
    if self.genGraphics:
       ...
       self.UMLmodel.itemconfig(new_obj.gf1.handler, stipple='')
       self.UMLmodel.itemconfig(new_obj.gf1.handler, fill='black')
       self.UMLmodel.itemconfig(new_obj.gf1.handler, text='Activate Button*')
       self.UMLmodel.itemconfig(new_obj.gf1.handler, width='0')
    ...
    # display
    self.obj40.display.setValue('<Any-ButtonRelease-1> HANDLED')
    ...
    if self.genGraphics:
       ...
       new_obj.layConstraints['scale'] = [1.0, 1.0]
       new_obj.layConstraints['Label Offset'] = [-168.0, -10.0]
    else: new_obj = None
    ...
    # trigger
    self.obj55.trigger.setValue('<ButtonPress-1>')

    # action
    self.obj55.action.setValue('event = eventhandler.get_event_params()\nevent._isHandled = True\n')
    ...
    # display
    self.obj55.display.setValue('<ButtonPress-1> HANDLED')

    self.obj55.graphClass_= graph_Hyperedge
    if self.genGraphics:
       new_obj = graph_Hyperedge(629.0,104.0,self.obj55)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("Hyperedge", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
       new_obj.layConstraints['Label Offset'] = [-73.0, 53.0]
       new_obj.layConstraints['scale'] = [1.0, 1.0]
    else: new_obj = None
    ...
    # text
    self.obj43.text.setValue('Button behaviour\n')
...
(self.obj38,self.obj55,[488.80634552792418, 116.43524907991582, 629.0, 184.0, 629.0, 104.0],"bezier", 3) )
    # Connections for obj39 (graphObject_: Obj2) named Activate Button*
    self.drawConnections(
...
(self.obj42,self.obj38,[291.00670445776228, 112.02160554679284, 472.01149796808687, 112.07819072029443],"bezier", 2) )
    ...
    self.drawConnections(
(self.obj55,self.obj38,[629.0, 104.0, 629.0, 24.0, 488.92020890121103, 107.49479166666664],"bezier", 3) )
...
newfunction = ButtonBehaviour_MDL
