from ASG_TransformationToolbar import *
...
def createNewASGroot(self):
   return ASG_TransformationToolbar(self, None)
...
def setConnectivity(self):
    ...
    self.entitiesInMetaModel['TransformationToolbar']=[]

    
def createNew_Model(self, wherex, wherey, screenCoordinates = 1):
   self.toClass = None
   self.fromClass = None
   new_semantic_obj = ASG_TransformationToolbar(self)
   ne = len(self.ASGroot.listNodes["ASG_TransformationToolbar"])
   ...
   self.UMLmodel.addtag_withtag("ASG_TransformationToolbar", new_obj.tag)
