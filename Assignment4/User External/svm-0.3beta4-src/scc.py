...
def print_usage():
  ...
  print '                      supported languages: java (default), cpp, csharp, python, tkinter'
  ...
  print '       parameters:'
  print '           "name=value"'
  print '       NOTE: tkinter is just Python with --ext on and generated code uses Tkinter '
  print '             for the polling loop in timed transitions (No threads!)'
...
try:
  if len(sys.argv)<2:
    ...
    elif code_type=="tkinter":
      if not command_only:
        from TkinterGenerator import PythonGenerator
        g=PythonGenerator(eventhandler, 1 ) # action_ext == True 
        g.generate()
      info["file"]="%s.py" % model_name
      info["compile"]="(N/A)"
      info["run"]="python %s.py" % model_name
